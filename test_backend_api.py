#!/usr/bin/env python
"""
Comprehensive backend API tests for DevLink application
"""
import requests
import json
import os
...
def test_ai_endpoints():
    ...
    try:
        ...
        if response.status_code == 200:
            data = response.json()
            print(f"   ✅ Email generated: {data.get('subject', 'No subject')[:50]}...")
        else:
    ...
    try:
        ...
        if response.status_code == 200:
            ...
            if isinstance(data, list) and len(data) > 0:
                ...
                print("   ⚠️ No businesses found (might be API key issue)")
        else:
            print("   ❌ Business generation failed")
    except Exception as e:
        print(f"   ❌ Business generation error: {e}")
...
def main():
    ...
    print("If any show ❌, check the Django server and .env configuration.")
